...
Summary_Dataset_Multivar_Relative_MNAU <- as.data.frame(matrix(nrow = length(dataframe_names), ncol = length(columns)))

names(Summary_Dataset_Multivar_Relative_MNAU) <- columns
rownames(Summary_Dataset_Multivar_Relative_MNAU) <- dataframe_names
...
for (df_name in dataframe_names) {
  ...
  Summary_Dataset_Multivar_Relative_MNAU[df_name, 1:length(values)] <- values
}
...
Summary_Dataset_Multivar_Relative_MNAU$Type <- sapply(rownames(Summary_Dataset_Multivar_Relative_MNAU), assign_type)

#"Type" column as the first column
Summary_Dataset_Multivar_Relative_MNAU <- Summary_Dataset_Multivar_Relative_MNAU[, c("Type", names(Summary_Dataset_Multivar_Relative_MNAU)[-ncol(Summary_Dataset_Multivar_Relative_MNAU)])]
...
Summary_Dataset_Multivar_Relative_MNAU <- Summary_Dataset_Multivar_Relative_MNAU %>%
  mutate(Individuals = individuals_dict[match(rownames(Summary_Dataset_Multivar_Relative_MNAU), names(individuals_dict))])

# Move "Individuals" to the third position
Summary_Dataset_Multivar_Relative_MNAU <- Summary_Dataset_Multivar_Relative_MNAU %>%
  select(1:2, Individuals, everything())

# Drop "Metacarpal", "Metatarsal", "Hand phalanx", and "Foot phalanx"
Summary_Dataset_Multivar_Relative_MNAU <- Summary_Dataset_Multivar_Relative_MNAU %>%
...
# Bind non-numeric columns back to the normalized data
Summary_Dataset_Multivar_Relative_MNAU <- Summary_Dataset_Multivar_Relative_MNAU %>%
  select(Type, AccumulationType, Individuals) %>%
  bind_cols(Summary_Dataset_Multivar_Relative_MNAU_normalized)


# Save dataset Summary_Dataset_Multivar_Relative_MNAU as CSV
write.csv(Summary_Dataset_Multivar_Relative_MNAU, "INPUT/DATA/Summary_Dataset_Multivar_Relative_MNAU.csv", row.names = TRUE)
...
Summary_Dataset_Used_Relative_MNAU <- Summary_Dataset_Multivar_Relative_MNAU[samples_used,]
...
Summary_Dataset_Used_Relative_MNAU = assign_clusters(Summary_Dataset_Used_Relative_MNAU)

# Save "used" dataset
write.csv(Summary_Dataset_Used_Relative_MNAU, "INPUT/DATA/Summary_Dataset_Used_Relative_MNAU.csv", row.names = TRUE)
